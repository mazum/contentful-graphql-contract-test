...
test('All contracts should be intact', function (done) {
  return readFile('./test/fixtures/contentTypes.json', 'utf8')
    ...
    .then((schema) => {
      return readdir('./test/queries/').then((filenames) => {
        return ({
          filenames: filenames,
          schema: schema
        })
    ...
    .then((object) => {
      ...
      object.filenames.forEach((filename) => {
        schemaTests.push(new Promise(resolve => {
          readFile(`./test/queries/${filename}`, 'utf8').then((query) => {
            const queryAST = parse(query)
            const errors = validate(object.schema, queryAST)
            const isValid = !errors.length
            if (errors.length) {
              console.log(
                `There is an issue with ${filename}. With actual error message as follows:\n${errors}`)
            }
